Feature: cli/install
  ...
  @slow
  Scenario: Install a module dependency from git and forge should be deterministic when not using qualified module name
    Given a file named "Puppetfile" with:
    """
    forge "http://forge.puppetlabs.com"

    mod 'stdlib', :git => 'https://github.com/puppetlabs/puppetlabs-stdlib.git', :ref => '3.0.0'
    mod 'test', :git => 'https://github.com/rodjek/librarian-puppet.git', :path => 'features/examples/test'
    """
    When I run `librarian-puppet install --verbose`
    Then the exit status should be 0
    And the file "modules/stdlib/Modulefile" should match /version *'3\.0\.0'/
    And PENDING the output should not contain "Executing puppet module install for puppetlabs/stdlib"
